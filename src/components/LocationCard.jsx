...
const LocationCard = ({ location }) => {
  return (
    <div className="relative rounded-2xl overflow-hidden shadow-lg group h-96 flex flex-col justify-end p-6 border border-gray-700 hover:border-brand-primary transition-all duration-300 transform hover:-translate-y-2 hover:shadow-2xl hover:shadow-brand-primary/20">
      <Image src={location.imageUrl} alt={location.name} layout="fill" objectFit="cover" className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent"></div>
      <div className="relative z-10 text-white">
        <h3 className="text-2xl font-bold">{location.name}</h3>
        <p className="text-gray-300 mt-2 text-sm">{location.description}</p>
        <Link 
          href={`/?listingType=buy&location=${encodeURIComponent(location.name)}`}
