...
export default function page() {
  ...
  // Only rent listings
  const rentOnly = MOCK_PROPERTIES.filter(
    (p) => p.type.toLowerCase() === "rent"
  );

  // Full filtering logic
  const filteredList = useMemo(() => {
    return rentOnly.filter((property) => {
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div className="relative h-[75vh] min-h-[500px] flex items-center justify-center text-center">
        ...
        <div className="relative z-10 px-4">
          <h1 className="text-4xl md:text-6xl font-bold text-white mb-4 tracking-tighter">
            Premium Dubai Rentals
          </h1>
          <p className="text-lg md:text-xl text-gray-300 max-w-3xl mx-auto">
            Explore hand-picked luxury rental properties in Dubai’s most iconic
            communities.
          </p>
      ...
      <PropertyListings
        ...
        subtitle="Explore the best homes and apartments available for rent."
      />
    </div>
  );
}
